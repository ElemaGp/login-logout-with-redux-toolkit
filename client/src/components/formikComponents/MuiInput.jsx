...
function MuiInput(props) {
  ...
  return (
    <div>
        <Field as={TextField} name={name} type={type}  variant="outlined" color="primary" label={label} sx={{width: {xs: 250, sm: 500}}} error={error} helperText={helperText}/>  {/*The "error" props makes the border red if it is true (ie when there's an error), the helper text displays the error message*/}
    </div>
  )
}
